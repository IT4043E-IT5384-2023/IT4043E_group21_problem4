...
def collect_contract_addresses(cur, required_chain_id: str, project_id: str, order_idx):
    cur.execute(f"SELECT contract_address FROM chain_{required_chain_id}.smart_contract WHERE project = '{project_id}'")
    return order_idx, [a[0] for a in cur.fetchall()]
...
def main():
    ...
    with ProcessPoolExecutor(max_workers=4) as executor, tqdm(total=len(kg)) as progress:
        futures = []
        for idx, k in enumerate(kg):
            project_id = k["id"]
            if project_id in contains and required_chain_id in deployed_chains[project_id]:
                future = executor.submit(collect_contract_addresses, 
                                         cur=cur, 
                                         required_chain_id=required_chain_id, 
                                         project_id=project_id, order_idx=idx)
                future.add_done_callback(lambda p: progress.update())
                futures.append(future)
        
        for future in futures:
            order_idx, addresses = future.result()
            k = kg[order_idx]
            k["contract_addresses"] = addresses
            projects.append(k)
...
if __name__ == "__main__":
    setup_logging(include_time=False)
    main()